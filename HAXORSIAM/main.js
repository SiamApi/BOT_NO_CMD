...
const config = require(`${__dirname}/siamcnf.json`);
const chalk = require("chalk");
const login = require('./includes/fca-siam');
const fs = require("fs");
...
let globalHostUrl; // Declare a global variable to store the host URL

app.use((req, res, next) => {
  globalHostUrl = req.protocol + '://' + req.get('host');
  next();
});
...
var configValue;
try {
    global.client.configPath = join(`${__dirname}/siamcnf.json`);
    configValue = require(global.client.configPath);
    logger.loader("Found siamcnf.json file!");
} catch (e) {
    return logger.loader(`siamcnf.json" file not found.${e}`, "error");
}
...
try {
    var appStateFile = resolve(join(`${__dirname}/siamfblogin.json`));
    ...
    logger.loader("Found the bot's siamfblogin file.")
} catch (e) {
    return logger.loader("Can't find the bot's siamfblogin file.", "error")
}
...
function onBot() {
    ...
    login(loginData, async (loginError, loginApiData) => {
        ...
        (async () => {
            const commandsPath = `${global.client.mainPath}/HAXORSIAM/main`;
            ...
            for (const command of listCommand) {
                try {
                    ...
                    if (envConfig) {
                        ...
                        var configPath = require('./siamcnf.json');
                    ...
                    global.loading(`${chalk.hex('#ff7100')(`[ MAIN ]`)} ${chalk.hex("#FFFF00")(config.name)} succes`, "LOADED");
                } catch (error) {
                    global.loading.err(`${chalk.hex('#ff7100')(`[ MAIN ]`)} ${chalk.hex("#FFFF00")(command)} fail `, "LOADED");
                }
            }
        })(),
        (async () => {
            const events = readdirSync(join(global.client.mainPath, 'HAXORSIAM/group')).filter(ev => ev.endsWith('.js') && !global.config.eventDisabled.includes(ev));
            console.log(chalk.blue('------------ GROUP CMD LOADING --------------'));
            for (const ev of events) {
                try {
                    const event = require(join(global.client.mainPath, 'HAXORSIAM/group', ev));
                    const { config, onLoad, run } = event;
                    if (!config || !config.name || !run) {
                        global.loading.err(`${chalk.hex('#ff7100')(`[ GROUP ]`)} ${chalk.hex("#FFFF00")(ev)} Module is not in the correct format. `, "LOADED");
                        continue;
                    }
                    if (global.client.events.has(config.name)) {
                        global.loading.err(`${chalk.hex('#ff7100')(`[ GROUP ]`)} ${chalk.hex("#FFFF00")(ev)} Module is already loaded!`, "LOADED");
                        continue;
                    }
                    ...
                    global.loading(`${chalk.hex('#ff7100')(`[ GROUP ]`)} ${chalk.hex("#FFFF00")(config.name)} loaded successfully`, "LOADED");
                } catch (err) {
                    global.loading.err(`${chalk.hex('#ff7100')(`[ GROUP ]`)} ${chalk.hex("#FFFF00")(command)} fail `, "LOADED");
                }
            }
        })();
        console.log(chalk.blue(`-------------- BOT START { SIAM RAHMAN } -------------`));
